...
allInputs.forEach(input => {
  ...
  try {
    ...
    try {
      ...
      console.error("[saveCurrentBoxData] No es JSON válido. Contenido crudo:", raw);
      throw new Error("Respuesta inválida del servidor");
    }

    if (!json.success) {
      console.error(`[saveCurrentBoxData] El servidor devolvió success:false → mensaje:`, json.message);
    ...
    console.error(`[saveCurrentBoxData] Excepción al intentar guardar Box ${boxNumber}:`, err);
  }
}
  ...
  function calculateDerivedValues() {
    const getValue = (selector) => parseFloat(document.querySelector(selector)?.value) || 0;
    ...
    if (balanceField) {
      ...
      balanceField.style.backgroundColor = balanceTotal >= 0
        ? (isDarkMode ? "#0070C0" : "#00A2E8")
        : (isDarkMode ? "#a31c1c" : "#ff0000");
  ...
  boxLinks.forEach((link) => {
    link.addEventListener("click", async (ev) => {
      ...
      // Quitar disabled de “Peso” y “Horas”
