...
<meta charset="UTF-8">
  ...
  <title>PRUEBAS Evolución de Enfermería</title>
...
#printArea #labelFirma,
@media print {
  #labelFirma {
    display: none !important;
  }
}
.campo textarea#firma {
  min-height: 40px;
  max-width: 100%;
}
  </style>
...
<body>
  ...
  <h1>PRUEBAS Evolución de Enfermería</h1>
  <div id="contenidoApp" style="display: none"></div>
  <div class="box-selector" id="boxSelector"></div>
  

  <a href="https://jolejuma.es/evolucion-uci_pruebas/index_balance_uci.php" class="icono-container">
...
<div class="campo">
  ...
  <textarea id="cardiovascular" disabled></textarea>
    ...
    <div class="campo">
  <label for="firma" id="labelFirma">Firma (Opcional)</label>
  <textarea id="firma" placeholder=""></textarea>
</div>
  ...
  <div id="overlayBloqueo">Seleccione Box</div> <!-- NUEVO== -->
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    deshabilitarCampos();            // bloquea todo  NUEVO
...
function deshabilitarCampos() {
  ...
  firma.disabled = true;
  firma.placeholder = "Seleccione un Box o informe guardado";
}
...
//===================================================
  ...
  return html;
  // Añadimos el valor de la firma como atributo personalizado para imprimir
const divRes = document.getElementById('resultado');
divRes.setAttribute('data-firma', datos.firma || '');
}
...
datos.firma = document.getElementById("firma").value.trim() || "";
  // Previsualizar
...
async function selectBox(boxNumber) {
  ...
  // Mostrar el indicador flotante con el número del box
  const indicador = document.getElementById("box-indicador-flotante");
  const numeroIndicador = document.getElementById("numero-box-seleccionado-fijo");

  if (indicador && numeroIndicador) {
  numeroIndicador.textContent = `Box-${boxNumber}`; // Agrega "Box-" antes del número
  indicador.style.display = "block"; // Muestra el indicador
}
...
async function cargarInformeDesdeLista(sel) {
  ...
  try {
    ...
    document.getElementById("firma").value = json.datos.firma || "";
    actualizarContadorTotal();
...
// ——————————————————————
  ...
  /* Re-carga si el navegador trae la página del Back/Forward-Cache  */
  window.addEventListener('pageshow', e => {
    if (e.persisted) location.reload();   // fuerza ida al servidor
  });   //NUEVO
