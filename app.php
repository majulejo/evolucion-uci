...
function saveLocal(boxNumber) {
  ...
  sessionStorage.setItem(
    `autosave_${currentUserId}_box${boxNumber}`,
...
function attachAutosaveListeners() {
  campos.forEach(id => {                                  // ← abre forEach
    const el = document.getElementById(id);
    if (!el) {
      console.error(`Elemento con ID ${id} no encontrado.`);
      return;
    }
    el.addEventListener('input', () => {                  // ← abre callback de addEventListener
      actualizarContadorTotal();
      clearTimeout(autosaveTimer);
      autosaveTimer = setTimeout(() => {                  // ← abre callback de setTimeout
        saveDraft();
        saveReport();  // si quieres servidor + informe real en paralelo
      }, DEBOUNCE_MS);                                    // ← cierra callback de setTimeout
    });                                                   // ← cierra callback de addEventListener
  });                                                     // ← cierra forEach
}                                                         // ← cierra función attachAutosaveListeners
...
function clonarInforme() {
 ...
 async function generarInforme() {
  ...
  // 1) Recojo los valores de los textarea
  const datos = {};
  campos.forEach(c => {
  ...
  // 3) Generar un UUID si no existe
  if (!currentInformeId) {
    currentInformeId = crypto.randomUUID();
  }

  try {
    // 4) Guardar en la tabla "reports"
    ...
    if (d.success) {
      // 5) Si todo OK, borramos el draft de esta caja
      ...
      // 6) Actualizamos el ID por si el servidor lo ha cambiado
      currentInformeId = d.id || currentInformeId;

      // 7) Avisamos al usuario y recargamos el listado
  ...
  // 0) guarda localmente el anterior…
  if (selectedBox) saveLocal(selectedBox);

  // 1) UI…
  selectedBox = boxNumber;
  // … marca botones, muestra mensajes, habilita campos …
  habilitarCampos();

  // 2) intenta cargar borrador DE ESTA caja
  try {
    ...
    if (js.success) {
      currentInformeId = null;  // forzamos que sea nuevo al generar
      campos.forEach(id => {
  ...
  // 3) si no hay borrador, pedimos el último informe ya guardado…
  try {
    const res = await fetch(`get_latest_report.php?box=${boxNumber}`, {
      credentials: 'same-origin'
    });
  ...
  // 4) si tampoco hay informe, limpiamos
  currentInformeId = null;
  borrarDatos();
}
...
function saveReport() {
  ...
  fetch('save_report.php', {
    method:      'POST',
  credentials: 'include',      // en lugar de 'same-origin'
    headers:     { 'Content-Type': 'application/json' },
...
async function cargarListadoInformesGuardados() {
  ...
  try {
    const res  = await fetch('list_reports.php', { credentials: 'same-origin' });
    const json = await res.json();

    if (!json.success || !Array.isArray(json.reports)) return;

    json.reports.forEach(report => {
      const o = document.createElement('option');
      o.value       = report.id;
      o.textContent = `Box ${report.box} – ${report.fecha} ${report.hora}`;
      sel.appendChild(o);
    });

  } catch (err) {
    console.error('Error al cargar lista de informes:', err);
  }
}
...
async function cargarInformeDesdeLista(sel) {
  ...
  try {
    ...
    // 1) Lee como texto y muéstralo en consola
    const text = await res.text();
    console.log("Respuesta get_report.php:", text);
    ...
    try {
      ...
      console.error("JSON inválido recibido de get_report.php:", text);
      return alert("Respuesta inválida del servidor:\n" + text);
    }

    // 3) Ahora ya sabes que json es un objeto
    if (!json.success) {
      return alert(json.message || "Informe no encontrado");
    }

    // 4) Rellenas los campos sólo si success===true
    ...
    document.querySelector(`.box-selector button:nth-child(${json.box})`)
...
async function selectBox(boxNumber) {
  // 0) guarda localmente el box anterior
  if (selectedBox) saveLocal(selectedBox);

  // 1) UI: marca botón, mensajes, habilita campos…
  selectedBox = boxNumber;
  ...
  document.getElementById("numero-box-seleccionado-fijo").textContent = `Box ${boxNumber}`;
